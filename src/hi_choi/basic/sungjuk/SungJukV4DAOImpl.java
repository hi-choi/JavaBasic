...
import hi_choi.basic.jdbc.JDBCUtil;
...
public class SungJukV4DAOImpl implements SungJukV4DAO{
    ...
    private String insertSQL = " insert into sungjuk (name, kor, eng, mat,tot,mean,grd) " +
            " values (?,?,?,?,?,?,?) ";
    private String selectSQL =" select sjno, name, kor, eng, mat, mid(regdate,1,10) regdate from sungjuk s order by sjno ";
    private String selectOneSQL =" select * from sungjuk where sjno = ? ";
    private String updateSQL =" update sungjuk set name = ?, kor = ?, eng= ?, mat= ? " +
            " ,tot = ?, mean = ?, grd = ?, regdate = current_timestamp " + " where sjno = ? ";
    ...
    public SungJukVO selectOneSungJuk(int sjno) {
        ...
        PreparedStatement pstmt = conn.prepareStatement(selectOneSQL)
            ...
            ResultSet rs = pstmt.executeQuery();
                while (rs.next()){
                    sj = new SungJukVO(rs.getString(2), rs.getInt(3),rs.getInt(4),rs.getInt(5));
                    sj.setSjno(rs.getString(1));
